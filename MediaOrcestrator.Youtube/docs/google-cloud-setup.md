...
| `token_path`    | Путь к файлу для хранения OAuth-токена (например, `C:\config\youtube_token.json`) |

При первом обращении к API откроется браузер для авторизации через Google-аккаунт. Токен сохранится в указанный файл и
