...
Не одна обложка на видос, а шаблон со слоями и профилями. Придумал новый визуальный стиль - перекатил пачкой все 200
видео,
...
Видно что качается, что льётся, сколько осталось. Не "программа повисла", а "идёт выгрузка, ещё 40%". Прогрессбары в
винде
...
Плагины - это DLL-ки в папке `ModuleBuilds/`. При старте `PluginLoader` проходит по ним reflection-ом, ищет
реализации `IPluginModule` и даёт каждому зарегистрировать свои сервисы в общий DI до `BuildServiceProvider`.
В сборках без `IPluginModule` авторегистрируются классы, реализующие `ISourceType` — обратная совместимость
со старым форматом. Новая площадка = новая DLL в папку. Без явной регистрации в хосте.
